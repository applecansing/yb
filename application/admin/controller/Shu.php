...
use think\Db;
use think\Request;
class Shu extends Conmmon{
 public function index(){
   $list = Db::name('shuji')->order('xu desc')->select();
...
public function add(){
 ...
 public  function del(){
  if(request()->isAjax()){
    $id = input('param.id');
    $row = Db::name('shuji')->where('id='.$id)->delete();
    if($row){
      return 1;
    }else{
      return 0;
...
public function edit(){
 if(request()->isAjax()){
    ...
    $res = Db::name('shuji')->where('id='.$id)->update($map);
    if($res){
   return 1;
  }else{
  return 0;
  ...
  $pid = input('param.pid');
  $sqlone = Db::name('shuji')->where('id='.$pid)->find();
