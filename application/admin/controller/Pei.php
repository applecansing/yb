...
use think\Db;
use think\Request;
class Pei extends Conmmon{
 public function index(){
   $list = Db::name('peixun')->order('xu desc')->select();
...
public function add(){
 ...
 public  function del(){
  if(request()->isAjax()){
    $id = input('param.id');
    $row = Db::name('peixun')->where('id='.$id)->delete();
    if($row){
      return 1;
    }else{
      return 0;
...
public function edit(){
 if(request()->isAjax()){
    ...
    $res = Db::name('peixun')->where('id='.$id)->update($map);
    if($res){
   return 1;
  }else{
  return 0;
  ...
  $pid = input('param.pid');
  $sqlone = Db::name('peixun')->where('id='.$pid)->find();
