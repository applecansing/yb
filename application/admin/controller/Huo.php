...
use think\Db;
use think\Request;
class Huo extends Conmmon{
public function index(){
   $list = Db::name('huodong')->order('xu desc')->select();
...
public function add(){
  if(request()->isAjax()){
   ...
   if(!empty($_FILES['uploadfile']['name'])){ 
     ...
     $sql = Db::name('huodong')->insert($map);
     if($sql){
      return 1;
     }else{
      return 0;
...
public  function del(){
  if(request()->isAjax()){
    $id = input('param.id');
    $row = Db::name('huodong')->where('id='.$id)->delete();
    if($row){
      return 1;
    }else{
      return 0;
...
public function edit(){
 if(request()->isAjax()){
  ...
  $res = Db::name('huodong')->where('id='.$id)->update($map);
    if($res){
   return 1;
  }else{
  return 0;
  ...
  $pid = input('param.pid');
  $sqlone = Db::name('huodong')->where('id='.$pid)->find();
...
public function product_down(){
    if(request()->isAjax()){
        ...
        $map['id'] = $ids;
        $map['status'] = 0;
        $res = Db::name('huodong')->update($map);
        if($res){
         return 1;
        }else{
        return 0;
