...
use think\Db;
use think\Request;
class Newss extends Conmmon{
public function index(){
   $list = Db::name('news_fa')->order('xu desc')->select();
...
public function add(){
  if(request()->isAjax()){
   ...
   if(!empty($_FILES['uploadfile']['name'])){ 
     ...
     $sql = Db::name('news_fa')->insert($map);
     if($sql){
     	return 1;
     }else{
     	return 0;
...
public  function del(){
  if(request()->isAjax()){
    $id = input('param.id');
    $row = Db::name('news_fa')->where('id='.$id)->delete();
    if($row){
    	return 1;
    }else{
    	return 0;
...
public function edit(){
 if(request()->isAjax()){
  ...
  $res = Db::name('news_fa')->where('id='.$id)->update($map);
  	if($res){
   return 1;
  }else{
	return 0;
  ...
  $pid = input('param.pid');
  $sqlone = Db::name('news_fa')->where('id='.$pid)->find();
...
public function product_down(){
    if(request()->isAjax()){
        ...
        $map['id'] = $ids;
        $map['status'] = 0;
        $res = Db::name('news_fa')->update($map);
        if($res){
         return 1;
        }else{
        return 0;
